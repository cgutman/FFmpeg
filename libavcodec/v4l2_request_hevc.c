...
static void fill_dec_params(struct v4l2_ctrl_hevc_decode_params *dec_params, const HEVCContext *h)
{
    ...
    *dec_params = (struct v4l2_ctrl_hevc_decode_params) {
    ...
    for (i = 0; i < V4L2_HEVC_DPB_ENTRIES_NUM_MAX && i < h->rps[ST_CURR_BEF].nb_refs; i++)
        dec_params->poc_st_curr_before[i] = get_ref_pic_index(h, h->rps[ST_CURR_BEF].ref[i], dec_params);

    for (i = 0; i < V4L2_HEVC_DPB_ENTRIES_NUM_MAX && i < h->rps[ST_CURR_AFT].nb_refs; i++)
        dec_params->poc_st_curr_after[i] = get_ref_pic_index(h, h->rps[ST_CURR_AFT].ref[i], dec_params);

    for (i = 0; i < V4L2_HEVC_DPB_ENTRIES_NUM_MAX && i < h->rps[LT_CURR].nb_refs; i++)
                                         ...
                                         av_unused uint32_t size)
{
    ...
    return ff_v4l2_request_reset_frame(avctx, h->cur_frame->f);
}
...
static int v4l2_request_hevc_queue_decode(AVCodecContext *avctx, int last_slice)
{
    ...
    if (ctx->decode_mode == V4L2_STATELESS_HEVC_DECODE_MODE_SLICE_BASED)
        return ff_v4l2_request_decode_slice(avctx, h->cur_frame->f, control, num_controls, controls->first_slice, last_slice);

    return ff_v4l2_request_decode_frame(avctx, h->cur_frame->f, control, num_controls);
}
...
static int v4l2_request_hevc_decode_slice(AVCodecContext *avctx, const uint8_t *buffer, uint32_t size)
{
    ...
    V4L2RequestControlsHEVC *controls = h->cur_frame->hwaccel_picture_private;
    V4L2RequestContextHEVC *ctx = avctx->internal->hwaccel_priv_data;
    V4L2RequestDescriptor *req = (V4L2RequestDescriptor*)h->cur_frame->f->data[0];
    ...
    if (ctx->decode_mode == V4L2_STATELESS_HEVC_DECODE_MODE_SLICE_BASED && slice) {
        ...
        ff_v4l2_request_reset_frame(avctx, h->cur_frame->f);
    ...
    if (ctx->start_code == V4L2_STATELESS_HEVC_START_CODE_ANNEX_B) {
        ret = ff_v4l2_request_append_output_buffer(avctx, h->cur_frame->f, nalu_slice_start_code, 3);
        if (ret)
            return ret;
    }

    ret = ff_v4l2_request_append_output_buffer(avctx, h->cur_frame->f, buffer, size);
    if (ret)
        return ret;
